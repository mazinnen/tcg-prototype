...
/* ---------------------------------------------------------
   デッキ初期化（DB版）
   deckJson = { territory: "xxx", cards: [{id, count}, ...], work: "GNO" など }
--------------------------------------------------------- */
async function initDeckFromList(deckJson, carddata, workId) {
  ...
  cards["territory"] = {
    ...
    imageOpen:  `../data/img/BDB/${carddata[territoryId].title}/${territoryId}_O.png`,
    imageClose: `../data/img/BDB/${carddata[territoryId].title}/${territoryId}_C.png`
  };
  ...
  deckJson.cards.forEach(entry => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      cards[uid] = {
        ...
        image: `../data/img/BDB/${carddata[id].title}/${id}.png`,
        backImage: `../data/img/BDB/back.png`
      };
    }
  });
...
function setDeckOrder(newOrder) {
  deckOrder = [...newOrder];
}

window.setDeckOrder = setDeckOrder;
